...
def evaluate_model(model, model_name, queries):
    ...
    accuracy = sum(1 for r in results if r['positive_similarity'] > r['negative_similarity']) / len(results)

    print(f"\n{'-'*80}")
    print(f"Summary Statistics:")
    print(f"{'-'*80}")
    print(f"Accuracy (positive > negative): {accuracy:.1%} ({int(accuracy * len(results))}/{len(results)})")
    print(f"Average positive similarity: {np.mean(positive_sims):.4f} ± {np.std(positive_sims):.4f}")
    print(f"Average negative similarity: {np.mean(negative_sims):.4f} ± {np.std(negative_sims):.4f}")
...
def main():
    ...
    if lora_results['avg_margin'] > baseline_results['avg_margin']:
        improvement = ((lora_results['avg_margin'] - baseline_results['avg_margin']) /
                      baseline_results['avg_margin'] * 100)
        print(f"✓ The LoRA adapter shows {improvement:.1f}% improvement in margin!")
        print(f"  This indicates better semantic understanding of domain-specific queries.")
    else:
        degradation = ((baseline_results['avg_margin'] - lora_results['avg_margin']) /
                      baseline_results['avg_margin'] * 100)
        print(f"✗ The LoRA adapter shows {degradation:.1f}% degradation in margin.")
        print(f"  This suggests the training data or approach needs further investigation.")

...
echo "🖥️  CPU-Optimized PII LoRA Training"
echo "=================================="
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --quick)
            ...
            ;;
        --help)
            echo "CPU-Optimized PII LoRA Training"
            echo ""
...
{
    echo "PII Detection LoRA - CPU Training Summary"
    echo "========================================="
    echo "Date: $(date)"
    echo "Models: ${MODELS[*]}"
...
test_cpu_model() {
    local model_name=$1
    local python_model_dir="lora_pii_detector_${model_name}_r${LORA_RANK}_token_model"
    local rust_model_dir="lora_pii_detector_${model_name}_r${LORA_RANK}_token_model_rust"
    ...
    if [[ -d "$python_model_dir" ]]; then
        ...
        if eval "$python_cmd" > "$python_test_log" 2>&1; then
            ...
            echo "  📊 Python Results: $entities_count entities detected, $low_confidence low confidence detections"
            echo "$model_name: Python Test OK ($entities_count entities, $low_confidence low conf)" >> "$SUMMARY_FILE"
        else
...
echo "   • bert-base-uncased is usually the most CPU-friendly and stable
   • roberta-base may have better PII detection accuracy"

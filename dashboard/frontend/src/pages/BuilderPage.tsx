...
type EditableListener = Listener & { timeout: string };
  ...
  useEffect(() => {
    const handleGuideDragMove = (e: MouseEvent) => {
      ...
      const newWidth = Math.min(
        GUIDE_MAX_WIDTH,
        Math.max(GUIDE_MIN_WIDTH, guideDragStartWidthRef.current + delta),
      );
  ...
  useEffect(() => {
    const handleDragMove = (e: MouseEvent) => {
      ...
      const containerWidth =
        contentRef.current?.offsetWidth ?? window.innerWidth;
      const maxWidth = Math.floor(containerWidth * OUTPUT_MAX_RATIO);
      const newWidth = Math.min(
        maxWidth,
        Math.max(OUTPUT_MIN_WIDTH, dragStartWidthRef.current + delta),
      );
    ...
    <div className={styles.page}>
      ...
      <div className={styles.toolbar}>
        ...
        <div className={styles.modeSwitcher}>
          <button
            className={
              mode === "visual" ? styles.modeBtnActive : styles.modeBtn
            }
      ...
      {guideOpen &&
        createPortal(
          <div
            ...
            onClick={() => {
              if (!isGuideDraggingRef.current) setGuideOpen(false);
            }}
          >
            <div
            ...
            >
              <div className={styles.deployModalHeader}>
                ...
                <span className={styles.deployDiffLabels}>
                  ...
                  <span className={styles.deployDiffLabelNew}>
                    After Deploy
                  </span>
                </span>
              </div>
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      for (let i = nav.length - 1; i >= 0; i--) {
                        const startLine =
                          nav[i].modifiedStartLineNumber ||
                          nav[i].originalStartLineNumber;
                      ...
                      const lastLine =
                        last.modifiedStartLineNumber ||
                        last.originalStartLineNumber;
              ...
              <div className={styles.deployModalFooter}>
                ...
                >
                  A backup of the current config will be created before
                  deployment.
                </p>
                <button className={styles.toolbarBtn} onClick={dismissDeploy}>
                  Cancel
                </button>
                <button
                  className={styles.toolbarBtnDeploy}
          ...
          <div className={styles.deployToast}>
            ...
            {deployResult && !deploying && (
              ...
              >
                ...
                <span className={styles.deployResultMsg}>
                  {deployResult.message}
                </span>
                <button
                  className={styles.deployResultDismiss}
                  onClick={dismissDeploy}
                >
                  <svg
                    width="12"
                    height="12"
      ...
      {(isDragging || isGuideDragging) &&
        createPortal(<div className={styles.dragOverlay} />, document.body)}
    </div>
  );
};
                    ...
                    <div key={i} className={styles.valItem}>
                      ...
                      {d.fixes?.map((fix, fi) => (
                        <button
                          ...
                          onClick={() => handleApplyFix(d, fix.newText)}
...
const GlobalSettingsEditor: React.FC<{
  ...
  const vllmEndpoints = endpoints.filter(
    (e) => e.backendType === "vllm_endpoint",
  );
  const providerProfiles = endpoints.filter(
    (e) => e.backendType === "provider_profile",
  );

  const updateListener = useCallback(
    (index: number, field: keyof EditableListener, value: string | number) => {
      setLocal((prev) => {
        const current = getListeners(prev, "listeners");
    ...
    <div className={styles.globalEditor}>
      ...
      <div className={styles.gsSection}>
        <div
          ...
          onClick={() => toggleCollapse("routing")}
        >
          <svg
          ...
          <div className={styles.gsSectionBody}>
                ...
                {listeners.map((listener, index) => (
                  ...
                  >
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        updateListener(index, "address", e.target.value)
                      }
                      placeholder="0.0.0.0"
                    />
                    <input
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        updateListener(index, "timeout", e.target.value)
                      }
                      placeholder="300s"
                    />
                    <button
                ...
                {["priority", "confidence"].map((s) => (
                  <label key={s} className={styles.gsRadio}>
                    <input
                      type="radio"
                      name="gs-strategy"
                      checked={getStr(local, "strategy") === s}
                      onChange={() => setField("strategy", s)}
                    />
                    <span>{s}</span>
                  </label>
                ))}
                ...
                {["low", "medium", "high"].map((e) => (
                  <label key={e} className={styles.gsRadio}>
                    <input
                      type="radio"
                      name="gs-effort"
                      checked={getStr(local, "default_reasoning_effort") === e}
                      onChange={() => setField("default_reasoning_effort", e)}
                    />
                    <span>{e}</span>
                  </label>
                ))}
            ...
            <div className={styles.gsRow}>
              ...
              <input
                ...
                onChange={(e) =>
                  setNestedField("looper", "endpoint", e.target.value)
                }
              ...
              {getBool(promptGuard, "enabled") && (
                <div className={styles.gsSubBody}>
                  ...
                  <div className={styles.gsRow}>
                    ...
                    <div className={styles.gsRadioGroup}>
                      {[
                        { label: "Candle (local)", val: "candle" },
                        { label: "vLLM (external)", val: "vllm" },
                      ].map((o) => (
                    ...
                    onChange={(e) => {
                      ...
                      if (e.target.checked) {
                        ...
                        setField("hallucination_mitigation", {
                          ...
                          fact_check_model: {
                            ...fcm,
                            threshold: getNum(fcm, "threshold", 0.7),
                          },
                          hallucination_model: {
                            ...hm,
                            threshold: getNum(hm, "threshold", 0.5),
                          },
                ...
                <div className={styles.gsRow}>
                  ...
                  <label className={styles.gsCheckbox}>
                    <input
                      ...
                      onChange={(e) =>
                        setNestedField("authz", "fail_open", e.target.checked)
                      }
                    />
      ...
      <div className={styles.gsSection}>
        <div
          className={styles.gsSectionHeader}
          onClick={() => toggleCollapse("observability")}
        >
          <svg
            className={styles.gsSectionChevron}
                  ...
                  <div className={styles.gsRow}>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        setField("observability", {
                          ...obs,
                          tracing: { ...tr, provider: e.target.value },
                        });
                      }}
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        setField("observability", {
                          ...
                          tracing: {
                            ...tr,
                            exporter: { ...exp, endpoint: e.target.value },
                          },
                        });
                      }}
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          setField("observability", {
                            ...
                            tracing: {
                              ...tr,
                              sampling: { ...samp, type: e.target.value },
                            },
                          });
                        }}
                ...
                <label className={styles.gsCheckbox}>
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      setField("observability", {
                        ...obs,
                        metrics: { ...met, enabled: e.target.checked },
                      });
                    }}
                  />
                    ...
                    <label className={styles.gsCheckbox}>
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          setField("observability", {
                            ...obs,
                            metrics: { ...met, windowed: e.target.checked },
                          });
                        }}
                      />
      ...
      <div className={styles.gsSection}>
        <div
          className={styles.gsSectionHeader}
          onClick={() => toggleCollapse("endpoints")}
        >
          <svg
            className={styles.gsSectionChevron}
              ...
              <div className={styles.gsEndpointTable}>
                ...
                {vllmEndpoints.map((ep) => (
                  <div key={ep.name} className={styles.gsEndpointRow}>
                    ...
                    <span className={styles.gsEndpointAddr}>
                      {getStr(ep.fields, "address", "—")}:
                      {getStr(ep.fields, "port", "—")}
                    </span>
                    <span className={styles.gsEndpointType}>
                      {getStr(ep.fields, "type", "vllm")}
                    </span>
                ...
                {providerProfiles.map((pp) => (
                  <div key={pp.name} className={styles.gsEndpointRow}>
                    ...
                    <span className={styles.gsEndpointAddr}>
                      {getStr(pp.fields, "base_url", "—")}
                    </span>
                    <span className={styles.gsEndpointType}>
                      {getStr(pp.fields, "type", "openai")}
                    </span>
                    <span>—</span>
                  </div>
                ))}
